...
var app = (function () {
    ...
    /* ../../../tic-tac-toe-svelte/src/Square.svelte generated by Svelte v3.52.0 */

    const file = "../../../tic-tac-toe-svelte/src/Square.svelte";
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t = text(t_value);
    			attr_dev(button, "class", "square svelte-vep88b");
    			add_location(button, file, 27, 2, 481);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    class Square extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance$1, create_fragment$1, safe_not_equal, { value: 0, handleClick: 1 });
    ...
    // (211:0) <Canvas width={window.innerWidth} height={window.innerHeight}    on:mousemove={handleMouseMove}>
    function create_default_slot(ctx) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "slot",
    		source: "(211:0) <Canvas width={window.innerWidth} height={window.innerHeight}    on:mousemove={handleMouseMove}>",
    		ctx
    	});
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	function drawGrid(width, height, context) {
    		for (let y = -3 * r; y + r * Math.sin(a) < height + r; y += r * Math.sin(a)) {
    			for (let x = -r, j = 0; x + r * (1 + Math.cos(a)) < width + 3 * r; (x += r * (1 + Math.cos(a)), y += (-1) ** j++ * r * Math.sin(a))) {
    	...
    	function drawPaddle(context) {
    		context.save();
    		let x = mouse[0];
    		let y = mouse[1] - paddle_h / 2;
    		context.beginPath();
    		context.fillStyle = 'rgba(190, 162, 28, 0.95)';
    		context.strokeStyle = color_border;
    	...
    	function handleMouseMove({ clientX, clientY }) {
    		...
    		if (mouse[0] < border + 10) mouse[0] = border + 10;
    		if (mouse[0] > window.innerWidth - border - 10) mouse[0] = window.innerWidth - border - 10;
    		if (mouse[1] < border) mouse[1] = border;
    		if (mouse[1] > window.innerHeight - border - 100 - 10) mouse[1] = window.innerHeight - border - 100 - 10;
    	}
