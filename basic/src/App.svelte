<script>
  import { Canvas, Layer, t } from 'svelte-canvas';
    import { context, height, width } from '../../../../spaceship-svelte/src/game';
    import Square from '../../../../tic-tac-toe-svelte/src/Square.svelte';
  ...
  $: render = ({ context, width, height }) => {
    ...
    drawGrid(width, height, context, $t);
  ...
  function handleMouseMove ({ clientX, clientY }) {
    ...
    if (mouse[0] < border + 10)
      mouse[0] = border + 10;
    if (mouse[0] > window.innerWidth - border - 10)
      mouse[0] = window.innerWidth - border - 10;
    if (mouse[1] < border)
      mouse[1] = border;
    if (mouse[1] > window.innerHeight - border - 100 - 10)
      mouse[1] = window.innerHeight - border - 100 - 10;
	}
</script>

<!-- //TODO have the background on different layer than the paddle and ball -->
